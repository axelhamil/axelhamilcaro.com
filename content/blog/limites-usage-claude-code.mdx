...
faq:
  ...
  - question: "Une clé API personnelle contourne-t-elle la fenêtre de 5 heures ?"
    answer: "Oui. La documentation de Claude Code établit un ordre de priorité entre identifiants, dans lequel ANTHROPIC_API_KEY l'emporte sur l'authentification par abonnement. Vous basculez alors sur les limites API classiques, exprimées en requêtes et tokens par minute selon le palier de votre organisation, sans fenêtre glissante ni plafond hebdomadaire. En contrepartie l'usage est facturé à la consommation et sort du forfait Pro ou Max."
...
## Quels plans sont concernés, et peut-on y échapper ?
...
Il existe une vraie porte de sortie, et elle est documentée. Claude Code applique un [ordre de priorité entre vos identifiants](https://code.claude.com/docs/en/authentication) : si `ANTHROPIC_API_KEY` est défini dans votre environnement, cette clé l'emporte sur l'authentification par abonnement. Vous quittez alors le régime de la fenêtre glissante pour celui des limites API classiques, exprimées en requêtes et en tokens par minute selon le palier de votre organisation. Plus de fenêtre de 5 heures, plus de plafond hebdomadaire, mais une facturation à l'usage qui n'a plus rien à voir avec le forfait.

Entre les deux, Anthropic propose d'acheter du dépassement : une fois la limite atteinte, l'usage supplémentaire est facturé au tarif API standard. C'est désactivé par défaut et cela s'active dans les réglages de facturation. Un détail intéressant, l'API vous dit où vous en êtes sur ce point aussi. Le header `anthropic-ratelimit-unified-overage-status` vaut `rejected` quand le dépassement est refusé, et `overage-disabled-reason` en donne la cause, par exemple `org_level_disabled` quand un administrateur l'a bloqué pour toute l'organisation.
...
### Une clé API personnelle contourne-t-elle la fenêtre de 5 heures ?

Oui. La documentation de Claude Code établit un ordre de priorité entre identifiants, dans lequel `ANTHROPIC_API_KEY` l'emporte sur l'authentification par abonnement. Vous basculez alors sur les limites API classiques, exprimées en requêtes et tokens par minute selon le palier de votre organisation, sans fenêtre glissante ni plafond hebdomadaire. En contrepartie l'usage est facturé à la consommation et sort du forfait Pro ou Max.
