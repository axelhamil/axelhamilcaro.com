...
Sur la stack, fuyez les choix dogmatiques. La bonne technologie se justifie par le besoin, le profil de l'équipe et la trajectoire du produit, pas par la mode. Mon dernier MVP en date tourne sur une stack découplée Vite, React et Hono parce que c'est ce que le produit demandait, quand un autre appellerait plutôt [Next.js, React et Node](/services/developpeur-nextjs-freelance). Je couvre ces deux mondes avec PostgreSQL, ce qui me permet d'assurer ensuite une [maintenance applicative](/tma) cohérente sur le même périmètre.
...
- La **propriété du code**, avec cession des droits dans le contrat, hébergement du dépôt et de l'infrastructure sur vos comptes, et accès complet de votre côté. C'est le piège le plus fréquent sur un premier projet, et le plus coûteux quand il se referme.
